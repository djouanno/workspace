...
public class PhonyArrayList<E> extends AbstractList<E> implements List<E>,
	...
	private boolean batchRemove(Collection<?> c, boolean complement) {
		final Object[] elementDataTmp = elementData;
		int r = 0, w = 0;
		...
		try {
			...
			for (Object object : c) {
				for (int i = 0; i < size(); i++) {
					if (object.getClass() != elementData[i].getClass())
						throw new ClassCastException();
				}
			}
			for (; r < size; r++)
				if (c.contains(elementData(r)) == complement)
					elementDataTmp[w++] = elementData(r);
		} finally {
			// Preserve behavioral compatibility with AbstractCollection,
			// even if c.contains() throws.
			if (r != size) {
				System.arraycopy(elementData, r, elementDataTmp, w, size - r
						+ 1);
				w += size - r + 1;
			}
			if (w != size) {
